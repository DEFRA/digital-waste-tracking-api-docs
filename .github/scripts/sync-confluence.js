...
const authHeader =
  'Basic ' +
  Buffer.from(`${CONFLUENCE_USER}:${CONFLUENCE_API_TOKEN}`).toString('base64')
...
async function confluenceFetch(pathAndQuery, options = {}) {
  ...
  if (!res.ok) {
    throw new Error(
      `${options.method || 'GET'} ${pathAndQuery} failed: ${res.status} ${await res.text()}`
    )
  }
  return res
}
...
async function findExistingAttachmentId(filename) {
  const res = await confluenceFetch(
    `/${PAGE_ID}/child/attachment?filename=${encodeURIComponent(filename)}`
  )
...
async function fetchInlineComments() {
  ...
  return comments
    .filter(
      (c) => c.extensions.location === 'inline' && c.extensions.inlineProperties
    )
    .map((c) => ({
      id: c.id,
...
function restoreCommentMarkers(html, comments) {
  ...
  if (orphaned.length > 0) {
    console.log(
      `${orphaned.length} comment(s) orphaned - their anchor text no longer appears on the page:`
    )
    orphaned.forEach((c) =>
      console.log(`  ${c.id}: ${JSON.stringify(c.sel.slice(0, 60))}`)
    )
  }
  return body
}
...
async function main() {
  ...
  const current = await (
    await confluenceFetch(`/${PAGE_ID}?expand=version`)
  ).json()
