...
- `PUT /deliveries/{deliveryId}` — soft-delete guarded by `revision` on `deliveries`.
- `PUT /deliveries/{deliveryId}/receipt` — amendment guarded by `revision` on `receipt-events`.
...
- `hazardousWasteConsignmentCode` is needed here so that `POST /deliveries` can check the hazardous constraint ([D-010](../decisions.md#d-010)) with a single read on `movement-creations`.
...
Full-document snapshot of `deliveries` before each successful `PUT /deliveries/{deliveryId}` soft-delete toggle.
...
One document per `deliveryId`. Created by `POST /deliveries/{deliveryId}/receipt`.

```javascript
{
  _id: String,                 // deliveryId — 1:1 with deliveries (D-015)
  deliveryId: String,          // FK to deliveries
  revision: Number,            // incremented on PUT /deliveries/{deliveryId}/receipt
  recordedAt: Date,
  lastUpdatedAt: Date,
  traceId: String,
  submittingOrganisation: {
...
Full-document snapshot of `receipt-events` before each successful `PUT /deliveries/{deliveryId}/receipt` amendment.
...
3. Check closed-sequence rule ([D-029](../decisions.md#d-029)): query `deliveries` for any document whose `movementIds` includes this `movementId`. If found, the collection sequence is closed — reject with `BusinessRuleViolation`.
...
Cross-collection reads: `movement-creations` (existence + deleted guard), `deliveries` (closed-sequence check), `collection-events` (ordering check + sequence derivation). The `movement-creations` update in step 6 requires a multi-document write if both the `collection-events` insert and the state update are to be atomic.
...
1. Validate Joi schema.
2. Read `deliveries` by `deliveryId`.
   - `DELIVERY_NOT_FOUND` (404) if absent.
   - `BusinessRuleViolation` (400) if `isDeleted: true`.
3. Check for duplicate receipt: query `receipt-events` by `deliveryId`. If a document already exists, return `ValidationError` ("Receipt already recorded — use `PUT /deliveries/{deliveryId}/receipt` to amend").
4. D-006 carrier cross-check ([D-006](../decisions.md#d-006)): compare `payload.carrier` against `deliveries.carrier`. Mismatch → push a `CARRIER_MISMATCH` warning to `validation.warnings`. Not a hard error.
5. D-036 authoring: `submittingOrganisation` is stamped from the authenticated caller and stored on the receipt document.
6. `insertOne` into `receipt-events`. The `_id` is the `deliveryId` — a duplicate-key error here means a concurrent `POST` already completed; the application layer catches this and returns the same `ValidationError` as step 3.
7. Return `{ warnings }`.

Cross-collection reads: `deliveries` (one read — provides existence check, deleted guard, and the carrier needed for D-006 in a single document). No additional Movement reads are needed for the carrier check because the carrier is stored directly on `deliveries`.
...
### `PUT /deliveries/{deliveryId}` (soft-delete)

1. Read `deliveries` (existence guard).
2. If `isDeleted: true`, check `receipt-events` — reject if a receipt exists ([D-009](../decisions.md#d-009): a Delivery cannot be deleted once a receipt has been recorded).
3. D-036 authorisation: `submittingOrganisation` on `deliveries` must match authenticated caller.
4. Snapshot `deliveries` to `deliveries-history`.
5. `updateOne` on `deliveries` with `{ _id, revision }` filter; set `isDeleted`; increment `revision`.
6. Return `HTTP 200`.
...
- `{ _id: 1 }` — implicit; unique
- `{ deliveryId: 1 }` — unique
- `{ movementIds: 1 }` — reverse lookup: which Delivery covers a given Movement
...
- `{ _id: 1 }` — implicit; unique; keyed on `deliveryId`
- `{ deliveryId: 1 }` — unique
- `{ 'submittingOrganisation.defraCustomerOrganisationId': 1 }`
- `{ traceId: 1 }`
...
- **State management decision.** Option B1 (denormalised `state` on `movement-creations`) introduces multi-document transaction requirements on every collection-event write and every delivery-creation write. Option B2 (read-time derivation) avoids transactions but makes state-filtered queries expensive. A decision on which approach to take is needed before implementation.

- **Transaction scope.** Two of the four POST write paths require coordinated writes across collections: `POST /movements/{id}/collection` (insert `collection-events` + update `movement-creations.state`) and `POST /deliveries` (insert `deliveries` + update `movement-creations.state` for each referenced Movement). Whether to wrap these in MongoDB multi-document transactions or to accept a small window of state inconsistency with an eventual-consistency repair path needs deciding before production.
