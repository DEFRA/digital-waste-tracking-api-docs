...
- Keep Movement and Delivery as separate aggregates because the API and decisions already separate them.
...
- a receipt write for a `deliveryId` that does not exist
...
```javascript
{
  ...
  reason: String, // e.g. MOVEMENT_NOT_FOUND, DELIVERY_NOT_FOUND, ORG_MISMATCH
  traceId: String,
  timestamp: Date,
...
- `deliveryIds[]` is included for now because it may make read models simpler, but it should be treated as derived from `deliveries.movementIds[]`.
...
Snapshot of the full previous Delivery document before each successful mutation of an existing Delivery aggregate.
...
- `PUT /deliveries/{deliveryId}/receipt`
  - mutate `receipt`
  - increment `revision`
  - write prior snapshot to `deliveries-history`
...
- Delivery as a first-class aggregate spanning one or more `movementIds`
- receipt belonging to a Delivery rather than directly to a Movement
...
- Whether `deliveryIds[]` on Movement proves worth the denormalisation cost.
